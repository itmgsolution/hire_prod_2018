...
class ThaiFormat {
    ...
    /*==================Datetime=====================*/
    public static function date_format(DateTime $date, $format, $withArabicNumber=false) {
        $thaidate = array(
        ...
        if ($withArabicNumber){
        	return $date->format(implode($newformat));
        }else{
    ...
    public static function number_tonumbaht($number) {
        if (!is_numeric($number)) {
            return 'Cann\'t convert.';
        }

        $num = explode('.', strval($number));
        $numtext = self::thainum(number_format($num[0], 0, ".", ","));
        $dectext = (count($num) > 1) ? (strlen($num[1]) <= 2) ? ' บาท ' . self::thainum((strlen($num[1]) == 1) ? $num[1] . '0' : $num[1]) . ' สตางค์' : '.' . self::thainum($num[1]) . ' บาท' : ' บาท';

        return "{$numtext}{$dectext}";
    }
