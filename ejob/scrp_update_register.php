<?php
	...
	if($mode == "new"){
		...
		if($user_count > 0){			
			...
			$sqll = "select * from users where user_id = '$this_id' ";
			...
			header("location: view_register.php?id=$this_id");
		}
		...
		//yoes 20180320 -> move this to "mail confirmed"
		...
		for($i = 2; $i < 10 ; $i++){
			...
			if($_POST["register_contact_name_2".$i]){
				array_push($special_fields, 'FirstName');
				array_push($special_values, "'".doCleanInput($_POST["register_contact_name_2".$i])."'");
			}
			if($_POST["register_contact_lastname_2".$i]){
				array_push($special_fields, 'LastName');
				array_push($special_values, "'".doCleanInput($_POST["register_contact_lastname_2".$i])."'");
			}
			if($_POST["register_contact_phone_2".$i]){
				array_push($special_fields, 'user_telephone');
				array_push($special_values, "'".doCleanInput($_POST["register_contact_phone_2".$i])."'");
			}
			if($_POST["register_position_2".$i]){
				array_push($special_fields, 'user_position');
				array_push($special_values, "'".doCleanInput($_POST["register_position_2".$i])."'");
			}
			if(count($special_fields) > 0){
				
				array_push($special_fields, 'user_id');
				array_push($special_values, "'$this_id'");				
				
				array_push($special_fields, 'seq');
				array_push($special_values, "'$i'");
				
				$the_sql = generateInsertSQL($_POST,"users_contacts",$input_fieldss,$special_fields,$special_values, "replace");
				//echo $the_sql; 
