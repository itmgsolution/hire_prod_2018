<?php
	...
	if($mode == "new" && ($sess_accesslevel == 1 || $sess_can_manage_user)){ //yoes 20141007 ---> also $sess_can_manage_user
		...
		if(!$is_owner && ($sess_accesslevel == 1 || $sess_accesslevel == 2 || $sess_can_manage_user || $sess_accesslevel == 3)){
			...
			if($_POST["AccessLevel"] == 4 && $_POST["user_enabled"] == 1 && !$_POST["user_enabled_origin"]){
			...
			$count_old_pass = getFirstItem("select count(*) from users where user_id = '$sess_userid' and user_password = '$old_pass'");
			...
			$special_values = array(
							"'".doCleanInput($_POST["user_password_new_1"])."'"
							);
